# PulseForge
### AI cost and latency optimizer with real-time token profiling and smart routing.
...
> Part of my Senior Hybrid Engineer 2026 portfolio (`AX-03`). Built on the "Antigravity" model — logic, state, and UI run locally in Docker while heavy reasoning is offloaded to cloud APIs, so the whole system runs on modest hardware.

## 🚀 Quick Start
```bash
# 1. Clone
git clone https://github.com/Kimosabey/pulse-forge.git
cd pulse-forge
...
## ✨ Key Features
- Real-time token / latency / cost profiling
- Smart model routing to cut cost
